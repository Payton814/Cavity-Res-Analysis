...
def cavity_analysis(dataframe, Sample_Aeff = None, plot = False, holder_params = None,
    ...
    from getEpsError import getEpsError
    ...
    f0_theory = 300/2*np.sqrt(1/dataframe['WG_d (mm)'][0]**2 + dataframe['Mode'][0]**2/dataframe['WG_length (mm)'][0]**2)
    f3 = dataframe['Frequency (GHz)']
    sample_name = dataframe['Sample'][0]
    sample_height = np.array(dataframe['height (mm)'])
    Q = np.array(dataframe['Q'])

    base_l = dataframe['Length (mm)'][0] - dataframe['Length wo base (mm)'][0] 
    ...
    #print(p)
    m = p[0]
    #print(dataframe['Sample'][0], dataframe['Mode'][0], m)
    b = p[1]
    #print(b)
    
    #if (Solid == False):
     #   print(dataframe['Sample'][0], dataframe['Mode'][0], b)
    mi = pi[0]
    bi = pi[1]
    #print(b, bi, np.sqrt(b**2 + bi**2))
    #if (Solid == False):
    #    print("b variance: ", V[1,1]/b)

    #print("Variance Matrix: ", V)

    eps_r = []
    Del_eps_r_list = []
    eps_i = []
    '''if (dataframe['Mode'][0] == 9):
        a = 1
        #a = 1.0569 ## For Rexolite 
        a = 1.07595 ## For PET
    elif (dataframe['Mode'][0] == 7):
        a = 1
        #a = 1.02965 ## For Rexolite
        a = 1.039585 ## For PET
    elif (dataframe['Mode'][0] == 5):
        a = 1
        #a = 1.0135 ## For Rexolite
        a = 1.011105 ## For PET
    elif (dataframe['Mode'][0] == 3):
        a = 1
        #a = 1.00055 ## For Rexolite
        a = 0.99576 ## For PET'''
    if (holder_params == None):
        try:
    ...
    for ii in range(len(x)):
        #print('Using a = ' + str(a) + ' for intermediate step')
